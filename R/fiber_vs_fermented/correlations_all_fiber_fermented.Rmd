...
cazyme_host_proteomics_diff_tp7_join <- full_join(cazyme_sumbins_diff_tp7_na_remove,proteo_host_diff_NOFILT_tp7) %>% 
  ...
  select(-Participant,-Group) %>% 
  as.matrix() 
res <- rcorr(cazyme_host_proteomics_diff_tp7_join,type="spearman")
...
corr_heatmap_function <- function(df1,df2,ha_annot){
  ...
  # df2=proteo_host_diff_tp7_annotated
  # df1=cazyme_na_labels_removed
  # ha_annot=ha_proteo_host
...
cazyme_sumbins_na_labels_removed <- cazyme_sumbins_diff_tp7 %>% 
  gather(key=CAzyme,value=scaled_diff_tp7,-Participant,-Group) %>% 
  separate(col=CAzyme,into=c("CAzyme_bin","sub_family"),sep = "[(\\_]",remove=FALSE) %>% 
  filter(CAzyme_bin!="NA") %>% 
  select(Participant,Group,CAzyme,scaled_diff_tp7) %>% 
...
labels_df_new <- bind_rows(get_labels_function("Host","Cytokines",immune_diff_tp6_cytokine) ,
                       get_labels_function("Host","Cell_freq",immune_diff_tp6_cell_freq), 
                       get_labels_function("Host","Endog_sig",immune_diff_tp6_endog_sig), 
                       get_labels_function("Host","Sig_cap",immune_diff_tp6_sig_cap),
                       get_labels_function("Host","Proteo_host",proteo_host_diff_NOFILT_tp7), 
                       get_labels_function("Microbe","ASVs",asv_filt_rank_diff_tp7), 
                       get_labels_function("Microbe","Alpha_div",alpha_diversity_diff_tp7), 
                       get_labels_function("Microbe","SCFAs",scfa_diff_tp7), 
                       get_labels_function("Microbe","Proteo_microbe",proteo_microbe_diff_tp7), 
                       ...
                       get_labels_function("Diet","Dietary_intake",nutrients_data_diff_tp7))
ha_proteo_host = c("Immunoglobulin"='#1b9e77',
                   "Enzyme"='#d95f02',
                   "Structural"='#7570b3',
                   "Actin"='#e7298a',
                   "Defensin"='#66a61e',
...
df_join <- full_join(cazyme_sumbins_diff_tp7,proteo_host_diff_tp7_annotated)
raw_matrix <- df_join %>% 
  select(-Participant,-Group) %>% 
  as.matrix %>% 
  scale()
...
res2 <- cor(raw_matrix,method = "spearman")
corr_values <- flattenCorrMatrix(res$r, res$P) %>% 
  left_join(.,labels_df_new,by = c("row" = "Parameter")) %>% 
  dplyr::rename(row_group=data_group,row_type=data_type) %>% 
  left_join(.,labels_df_new,by = c("column" = "Parameter")) %>% 
  dplyr::rename(col_group=data_group,col_type=data_type) %>% 
...
ha = HeatmapAnnotation(type = type, annotation_name_side = "right",
                       col=list(type=ha_proteo_host))
type_row=sub("_.*", "", rownames(corr_spread)) 
ha_row= HeatmapAnnotation(type = type_row,
                          col=list(type=ha_cazymes),
                          which="row")
...
Plot host proteomics and cazymes with overlapping annotations
```{r}
#CAzyme annotation using dummy variables for each GH
cazyme_bin_annot_na_remove <- cazyme_bin_annot %>% 
  filter(!is.na(CAzyme_annot))
...
ha_cazyme_bins = HeatmapAnnotation(mucin=mucin_col,
                                   plant=plant_col,
                                   animal = animal_col,
                                   dextran=dextran_col,
                                   fungal=fungal_col,
                                   hmo=hmo_col,
                                   peptidoglycan=peptidoglycan_col,
                                   starch=starch_col,
                                   sucrose=sucrose_col,
                       col=list(
                         animal=c(`0`="#ffffff",
...
type_proteo_host=sub("_.*", "", colnames(select(proteo_host_diff_NOFILT_tp7_annot,-Participant,-Group)))
ha_proteo_host = HeatmapAnnotation(host_protein_type=type_proteo_host,
                       col=list(host_protein_type=c(
                        "Immunoglobulin"='#1b9e77',
                        "Enzyme"='#d95f02',
                        "Structural"='#7570b3',
                        "Actin"='#e7298a',
                        "Defensin"='#66a61e',
...
accession_protein_class_filter <- rownames(protein_class_host_proteo_df)

proteo_host_diff_tp7_protein_class_filt <-proteo_host_diff_NOFILT_tp7 %>% 
  select(c("Participant","Group",accession_protein_class_filter))

# df_join <- full_join(cazyme_sumbins_diff_tp7_na_remove,proteo_host_diff_NOFILT_tp7)
df_join <- full_join(cazyme_sumbins_diff_tp7_na_remove,proteo_host_diff_tp7_protein_class_filt)
...
hmap <- Heatmap(corr_spread,
      ...
      # col = col_fun,
      #col=addalpha(rev(brewer.pal(20, "Blues"))),
      # top_annotation = ha_proteo_host,
      top_annotation = ha_cazyme_bins,
      heatmap_legend_param=list(color_bar="continuous", legend_direction="vertical", legend_width=unit(5,"cm"), title_position="topcenter", title_gp=gpar(fontsize=8, fontface="bold")),
      #split=olink_baseline_flux_all$Study,
      row_title="CAzymes",
      row_title_side="left",
...
for(i in 1:length(row_order(hmap))){
  if(i==1){
    i=1
    clu=t(t(row.names(corr_spread[row_order(hmap)[[i]],])))
    out=cbind(clu,paste("cluster",i,sep=""))
    colnames(out)=c("Accession","Cluster")
  }
  else{
    clu=t(t(row.names(corr_spread[row_order(hmap)[[i]],])))
...
for(i in 1:length(column_order(hmap))){
  if(i==1){
    i=1
    clu=t(t(colnames(corr_spread[,column_order(hmap)[[i]]])))
    out=cbind(clu,paste("cluster",i,sep=""))
    colnames(out)=c("Description_accession","Cluster")
  }
  else{
    clu=t(t(colnames(corr_spread[,column_order(hmap)[[i]]])))
    clu=cbind(clu, paste("cluster", i, sep=""))
...
proteo_host_cluster_assignment <- out %>% 
  as.data.frame() %>% 
  separate(col=Description_accession,into=c("Description","Accession"),sep = "[(\\_]",remove=FALSE)
# write.csv(proteo_host_cluster_assignment,
#             "~/R/proteo_host_reference_cluster_assignment_all.csv",
#             row.names = FALSE,col.names = TRUE)
# write.table(filter(proteo_host_cluster_assignment,Cluster=="cluster1")$Accession,
#             "~/R/proteo_host_cluster1.csv",
#             row.names = FALSE,col.names = FALSE)
# write.table(filter(proteo_host_cluster_assignment,Cluster=="cluster2")$Accession,
#             "~/R/proteo_host_cluster2.csv",
#             row.names = FALSE,col.names = FALSE)
# write.table(filter(proteo_host_cluster_assignment,Cluster=="cluster3")$Accession,
#             "~/R/proteo_host_cluster3.csv",
#             row.names = FALSE,col.names = FALSE)
# write.table(filter(proteo_host_cluster_assignment,Cluster=="cluster4")$Accession,
#             "~/R/proteo_host_cluster4.csv",
#             row.names = FALSE,col.names = FALSE)
# write.table(unique(proteo_host_cluster_assignment$Accession),
#             "~/R/proteo_host_reference_all.csv",
#             row.names = FALSE,col.names = FALSE)
