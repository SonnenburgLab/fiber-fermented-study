...
compare_dend_function <- function(first_char,second_char,df_1,df_2){
  ...
  df_1=nutrients_data_diff_tp7
  df_2=cazyme_diff_tp7
...
```{r}
ppt_list <- intersect(nutrients_data_diff_tp7$Participant,cazyme_diff_tp7$Participant)

df_1_dend <- nutrients_data_diff_tp7 %>% 
  filter(Participant %in% ppt_list) %>% 
  arrange(Participant) %>% 
  select(-Participant,-Timepoint,-Group) %>% 
  # na.omit() %>% 
...
df_diff_temp_1 <- nutrients_data_diff_tp7 %>% 
  filter(Participant %in% ppt_list) %>% 
  select(-Timepoint,-Group)
df_diff_temp_2 <- cazyme_diff_tp7 %>% 
  filter(Participant %in% ppt_list) %>% 
  select(-Timepoint,-Group)

df_data_joined <- left_join(df_diff_temp_1,df_diff_temp_2) %>% 
  select(-Participant) %>% 
  as.matrix
...
res <- rcorr(df_data_joined, type = "spearman")
res2 <- cor(df_data_joined,method = "spearman")
corr_values <- flattenCorrMatrix(res$r, res$P) %>% 
  mutate(row_type=ifelse(row %in% colnames(nutrients_data_diff_tp7),"Nutrient","CAzyme"),
         col_type=ifelse(column %in% colnames(nutrients_data_diff_tp7),"Nutrient","CAzyme")) %>% 
