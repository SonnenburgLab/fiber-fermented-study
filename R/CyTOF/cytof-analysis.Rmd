...
frequency_data <- read.csv(paste(data_directory, "cytof_frequencies_cleaned.csv", sep = ""))

signaling_data <- read.csv(paste(data_directory, "cytof_signaling_data_for_significance_analysis.csv", sep = ""))

signaling_data_gather_clean <- signaling_data %>% 
  gather(key=Analyte,value = Result,B_cells_IkB:Classical_monocytes_pZap70) %>% 
  select(Participant,Group,Timepoint,Analyte,Result)
# saveRDS(signaling_data_gather_clean,paste(data_directory, "cytof_signaling_gather.rds", sep = ""))
...
findDelta(fermented_signaling_info[["model"]] , fdr = 0.1)

# input delta by looking manually

selected_fermented_model <- summary(fermented_signaling_info[["model"]], findDelta(fermented_signaling_info[["model"]] , fdr = 0.05)[2,1] )
...
selected_fiber_model <- summary(fiber_signaling_info[["model"]], 0.896052 )
...
feature_key <- read.csv(paste(data_directory, "cytof_signaling_feature_key.csv", sep = ""))
...
# new plot based on consistent style: 

test_palette <- c("lavender", "mediumpurple4", "lavender", "mediumpurple4")

ggplot(plot_df, aes(x = Feature, y = `Arcsinh ratio`)) + 
  geom_boxplot(aes(fill = population), alpha = 0.5) + coord_flip() +
  theme_bw()  + 
  geom_hline(yintercept = 0, linetype = 2) + 
  ggtitle("Significant changes CyTOF signaling in Fermented cohort") + scale_fill_manual(values = test_palette) + 
  scale_x_discrete(labels = plot_df$Protein)

ggsave(paste(plot_directory, "CyTOF_changes_fermented_updated_style.pdf", sep = ""), useDingbats = FALSE, height = 4, width = 6)
...
frequency_data_clean <- frequency_data %>% 
  select(Participant,Timepoint,Group,Feature, Frequency) %>% 
  dplyr::rename(Analyte=Feature,Result=Frequency)
# saveRDS(frequency_data_clean,paste(data_directory, "cytof_cell_freq_gather.rds", sep = ""))
...
for (feature in significant_frequency_fermented){
  ...
  plot_data <- frequency_data %>% 
          dplyr::filter(Group == "Fermented") %>% 
          dplyr::filter(Timepoint == 1 | Timepoint == 6) %>%
          dplyr::filter(Feature == feature) %>%
          select(Participant, Timepoint, Feature, Frequency) %>%
          spread(key = Timepoint, value = Frequency) %>%
          mutate(Frequency_fold_change = `6`/`1`)
 
  plotting_data[[feature]] <- plot_data
  
}

plot_df <- reduce(plotting_data, bind_rows)

ggplot(plot_df, aes(x = Feature, y = Frequency_fold_change)) +  
  geom_boxplot(aes(colour = Feature, fill = Feature), alpha = 0.5) + 
  theme_classic()  + 
  geom_hline(yintercept = 1, linetype = 2) + 
  ggtitle("Significant changes CyTOF frequencies in Fermented cohort") + scale_fill_brewer(palette="Dark2") +
  scale_colour_brewer(palette="Dark2")

ggsave(paste(plot_directory, "CyTOF_frequency_changes_fermented.pdf", sep = ""), useDingbats = FALSE, height = 4, width = 6)
```

Fiber cohort had no significant changes in cell frequency. 
...
# Fiber differences for inflammation groups

```{r}
group_df <- read_csv("../../data/combined_immune/fiber_immune_groups.csv")

group_df$Participant <- as.numeric(group_df$Participant)

signaling_data_with_groups <- signaling_data %>% left_join(group_df)

group1_data <- dplyr::filter(signaling_data_with_groups, Immune_group == "group1")
group2_data <- dplyr::filter(signaling_data_with_groups, Immune_group == "group2")
group3_data <- dplyr::filter(signaling_data_with_groups, Immune_group == "group3")


fiber_signaling_info_group1 <- run_sam(full_data = group1_data, 
                                diet = "Fiber", 
                                time1 = 1, 
                                time2 = 6, 
                                metadata_columns = colnames(select(signaling_data_with_groups, filename:Greater_10K, Immune_group)))

n_fiber <- length(fiber_signaling_info_group1[["selected_participants"]])

findDelta(fiber_signaling_info_group1[["model"]] , fdr = 0.1)

selected_fiber_model1 <- summary(fiber_signaling_info_group1[["model"]], 0.1 )

group1_signaling_significant <- selected_fiber_model1@mat.sig %>% 
                                          mutate(Feature = rownames(selected_fiber_model1@mat.sig)) %>% 
                                          dplyr::filter(`q.value` < .1) %>% 
                                          select(Feature, `d.value`, `q.value`) %>% mutate(Inflammation_group = "group1")

# group1 are all positive = red = increase in signaling

fiber_signaling_info_group2 <- run_sam(full_data = group2_data, 
                                diet = "Fiber", 
                                time1 = 1, 
                                time2 = 6, 
                                metadata_columns = colnames(select(signaling_data_with_groups, filename:Greater_10K, Immune_group)))

n_fiber <- length(fiber_signaling_info_group3[["selected_participants"]])

findDelta(fiber_signaling_info_group2[["model"]] , fdr = 0.1)

selected_fiber_model2 <- summary(fiber_signaling_info_group2[["model"]], 0.393295 )

group2_signaling_significant <- selected_fiber_model2@mat.sig %>% 
                                          mutate(Feature = rownames(selected_fiber_model2@mat.sig)) %>% 
                                          dplyr::filter(`q.value` < .1) %>% 
                                          select(Feature, `d.value`, `q.value`) %>% mutate(Inflammation_group = "group2")

# all negative are negative = blue = decrease in signaling

fiber_signaling_info_group3 <- run_sam(full_data = group3_data, 
                                diet = "Fiber", 
                                time1 = 1, 
                                time2 = 6, 
                                metadata_columns = colnames(select(signaling_data_with_groups, filename:Greater_10K, Immune_group)))
...
findDelta(fiber_signaling_info_group3[["model"]] , fdr = 0.364523)

selected_fiber_model3 <- summary(fiber_signaling_info_group3[["model"]], 0.365 )

group3_signaling_significant <- selected_fiber_model3@mat.sig %>% 
                                          mutate(Feature = rownames(selected_fiber_model3@mat.sig)) %>% 
                                          dplyr::filter(`q.value` < .1) %>% 
                                          select(Feature, `d.value`, `q.value`) %>% mutate(Inflammation_group = "group3")

# (almost) all negative = blue = decrease in signaling

SAM_inflammation_results <- bind_rows(bind_rows(group1_signaling_significant, group2_signaling_significant), group3_signaling_significant)

write_csv(SAM_inflammation_results, "../../data/CyTOF/saved_results/inflammation_groups_CyTOF_hits.csv")
