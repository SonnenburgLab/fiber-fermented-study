...
# added alternate to gather: 

food_seq_df_gather <- food_seq_df %>% 
  pivot_longer(names_to ="ASV",values_to ="ASV_count",cols = -c(SampleID:SampleID_merge))
...
# updating to remove "g__Raphanus"

food_taxatab_filt_taxa_collapse_gather_no_raph <- food_taxatab_filt_taxa_collapse_gather %>% 
  dplyr::filter(Genus_labeled != "g__Raphanus")

ggplot(food_taxatab_filt_taxa_collapse_gather_no_raph, aes(fill=Genus_labeled, y=ASV_count_sum, x=fermented_food_type)) + 
    geom_bar(position="fill", stat="identity")+
  theme_bw()+
  scale_fill_manual(values = mycolors)+
  theme(text = element_text(size=12))
...
food_ppt_tp_asvs_summarized <- food_ppt_tp_asvs_summarized %>% 
  mutate(all_ppt_percen_food_tp_100=all_ppt_percen_food_tp*100)
ggplot(filter(food_ppt_tp_asvs_summarized,Group=="Fermented"),aes(x=Timepoint,y=all_ppt_percen_food_tp_100))+
  geom_bar(stat="identity",position = "dodge",fill="#bebada")+
  theme_bw()+
  ylab("Number new ASVs found in fermented food")+
  scale_x_discrete(breaks=c(1,2,3,4,5,6,7,8,9),labels=c("-2","0","2", "4","6", "8","10","12","14"),name="Timepoint (weeks)") +
  geom_point(data=filter(food_ppt_tp_asvs_summarized,Group=="Fiber"),aes(x=Timepoint,y=all_ppt_percen_food_tp_100),color="black")+
  geom_line(data=filter(food_ppt_tp_asvs_summarized,Group=="Fiber"),aes(x=Timepoint,y=all_ppt_percen_food_tp_100,group=1),color="black",stat="identity",linetype="dashed")+
  theme(text = element_text(size=16))
ggsave(paste(save_path_figures,"percent_new_asvs_in_fermented_foods_time_bar_graph.pdf",sep=""),width = 5,height=6,useDingbats=FALSE)
```
