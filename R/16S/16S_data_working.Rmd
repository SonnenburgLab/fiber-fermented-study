...
ps_tip_glom <- tip_glom(ps_dada2_with_tree,h=0.2) #h=0.2; 202 unique clusters
...
zibr_tip_glom_fiber_fermented <- select(tip_glom_counts_spread_na_fill, c(Participant:Timepoint_value)) %>%
  ...
  filter(Timepoint_value !=2) #removing timepoint 1 because use tp 2 as the baseline covariant 
...
zibr_pvalue_df_fiber_inflamm <- zibr_run_function(filter(zibr_tip_glom_fiber_inflamm,Timepoint_value!=2),taxa_name_list)
...
ggplot(sigg_zibr_tip_glom_fiber_inflamm, aes(x=Timepoint_value, y=ASV_rel_ab, fill=Inflammation_group)) +
  geom_boxplot() + 
  scale_fill_manual(breaks=c("Fermented","Fiber"),values=c('#d6604d','#4393c3'))+
  scale_color_manual(values=c('#d73027','#4575b4'))+
  theme_classic()+
  scale_x_discrete(breaks=c(1,2,3,4,5,6,7,8,9),labels=c("-2","0","2", "4","6", "8","10","12","14"),name="Timepoint (weeks)") + 
  labs(x="Timepoint (months)",y="Relative Abundance")+
...
ggplot(filter(sigg_zibr_tip_glom_fiber_inflamm,rep_ASV_label!="ASV_27"), aes(x=Timepoint_value, y=ASV_rel_ab, fill=Inflammation_group)) +
  geom_boxplot() + 
  scale_fill_manual(breaks=c("Fermented","Fiber"),values=c('#d6604d','#4393c3'))+
  scale_color_manual(values=c('#d73027','#4575b4'))+
  theme_classic()+
  ...
  facet_wrap(~rep_ASV_label,scales="free")+
  scale_x_discrete(breaks=c(1,2,3,4,5,6,7,8,9),labels=c("-2","0","2", "4","6", "8","10","12","14"),name="Timepoint (weeks)") 
# ggsave(paste(save_path_figures,"fiber_inflamm_groups_tip_glom_abundance.pdf"),width = 5,height=4,useDingbats=FALSE)
