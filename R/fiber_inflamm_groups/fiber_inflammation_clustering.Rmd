...
tip_glom_counts$Timepoint %<>% as.integer
tip_glom_counts <- tip_glom_counts %>% 
  dplyr::rename(Timepoint_value=Timepoint)
...
#scfa data
scfa_data <- readRDS(paste(save_path_scfa,"scfa_data.rds",sep="")) %>% 
  filter(! Participant %in% c(8000,8012)) 
scfa_data_fiber <- scfa_data %>% 
  filter(Group=="Fiber" & Analyte !="Lactic acid") %>% #removing lactic acid because only measured in a subset of samples
  left_join(inflammationGroups,.)
```
...
siggenes_df <- summary(siggenes_output,0.20775)
...
ggplot(cytof_sig_fiber_inflamm_grps_sigg_diff,aes(x=Signaling,y=Value_diff,fill=Immune_group))+
  ...
  coord_flip()
#ggsave(paste(save_path_figures,"fiber_inflamm_groups_cytof_sigg_paired.pdf"),width = 6,height=5,useDingbats=FALSE)

#plot all significant chagnes in all groups 
cytof_sig_fiber_siggenes <- cytof_sig_fiber_gather %>%
  filter(Analyte  %in% unique(cytof_sig_fiber_inflamm_grps_sigg_diff$Analyte)) %>% 
  right_join(cytof_sig_key,.) %>% 
  select(-c(filename:SAMPLE_NAME),-Phase,-Greater_10K) 
cytof_sig_fiber_siggenes_diff <- cytof_sig_fiber_siggenes %>% 
  filter(Timepoint==1) %>% 
  dplyr::rename(Value_base=Value) %>% 
  select(-Timepoint) %>% 
  left_join(.,filter(cytof_sig_fiber_siggenes,Timepoint==6)) %>% 
  na.omit() %>% 
  mutate(Value_diff=Value-Value_base)

ggplot(cytof_sig_fiber_siggenes_diff,aes(x=Signaling,y=Value_diff,fill=Immune_group))+
  geom_boxplot()+
  facet_wrap(~Cell_type,scales="free")+
  geom_hline(yintercept =0,linetype="dashed")+
  theme_bw()+
  ...
  xlab("Immune Signaling")+
  coord_flip()
ggsave(paste(save_path_figures,"fiber_inflamm_groups_cytof_sigg_paired.pdf"),width = 15.2,height=9.35,useDingbats=FALSE)

#take out group 2 adn just show signfiicant in group 1 and 3
ggplot(filter(cytof_sig_fiber_siggenes_diff,Immune_group!="group2"),aes(x=Signaling,y=Value_diff,fill=Immune_group))+
  geom_boxplot()+
  facet_wrap(~Cell_type,scales="free")+
  geom_hline(yintercept =0,linetype="dashed")+
  theme_bw()+
...
ggplot(fiber_g_intake_inflamm_total_base,aes(x=Immune_group,y=fiber_g_total))+
  geom_boxplot()
```
...
ggplot(filter(samdf_diversity_fiber_inflamm,Timepoint %in% c(1:7)),aes(x=Timepoint,y=Observed,fill=Immune_group))+
  ...
  scale_x_discrete(breaks=c(1,2,3,4,5,6,7,8,9),labels=c("-2","0","2", "4","6", "8","10","12","14"),name="Timepoint (weeks)")+
  ylab("Observed ASVs")
#ggsave(paste(save_path_figures,"fiber_inflamm_groups_obs_alpha.pdf"),width = 6,height=5,useDingbats=FALSE)
samdf_diversity_fiber_inflamm$Timepoint %<>% as.factor
ggplot(filter(samdf_diversity_fiber_inflamm,Timepoint %in% c(2:7) & Immune_group %in% c("group1","group2")),aes(x=Timepoint,y=Observed,fill=Immune_group))+
  geom_boxplot(color="#525252",width=.65)+
  theme_bw()+
  scale_fill_manual(breaks=c("group1","group2","group3"),values=c("#d6604d","#fdae61","#4393c3"))+
  scale_x_discrete(breaks=c(1,2,3,4,5,6,7,8,9),labels=c("-2","0","2", "4","6", "8","10","12","14"),name="Timepoint (weeks)")+
  ylab("Observed ASVs")
#ggsave(paste(save_path_figures,"fiber_inflamm_groups12_obs_alpha.pdf"),width = 6,height=5,useDingbats=FALSE)

#post-hoc t-test for sig. different alpha diversity at each timepoint
samdf_diversity_fiber_inflamm_gp12 <- samdf_diversity_fiber_inflamm %>% 
  filter(Immune_group %in% c("group1","group2"))
post_hoc_pvalue_diversity_fiber_grp12=data.frame(Immune_group_comparison=c(),Timepoint=c(),pValue=c())
for(i in c(2:7)){
  df_tp <- filter(samdf_diversity_fiber_inflamm_gp12,Timepoint==i)
  df_tp_grp1 <- df_tp %>% filter(Immune_group=="group1")
  df_tp_grp2 <- df_tp %>% filter(Immune_group=="group2") 
  pVal=t.test(df_tp_grp1$Observed,df_tp_grp2$Observed)$p.value
  result=data.frame(Immune_group_comparison="group1vs2",Timepoint=i,pValue=pVal)
  post_hoc_pvalue_diversity_fiber_grp12=bind_rows(post_hoc_pvalue_diversity_fiber_grp12,result)
}
...
t.test(filter(samdf_diversity_fiber_inflamm_base,Immune_group=="group2")$baseline_obs,
       filter(samdf_diversity_fiber_inflamm_base,Immune_group=="group3")$baseline_obs,)

ggplot(samdf_diversity_fiber_inflamm_base,aes(x=Immune_group,y=baseline_obs,fill=Immune_group))+
  geom_boxplot()+
  theme_bw()+
  scale_fill_manual(breaks=c("group1","group2","group3"),values=c("#d6604d","#fdae61","#4393c3"))+
  theme(text = element_text(size = 16))
ggsave(paste(save_path_figures,"fiber_inflamm_groups_baseline_obs_alpha.pdf"),width = 6,height=5,useDingbats=FALSE)
...
#Fiber inflamm groups 2 and 3
tip_glom_counts_spread_fiber_grp23 <- tip_glom_counts_spread_fiber %>% 
  filter(Immune_group %in% c("group2","group3"))
taxa.raw <- select(tip_glom_counts_spread_fiber_grp23, -c(Immune_group:Timepoint_value))
filter.index1 <- apply(taxa.raw,2,function(X){sum(X>0)>0.25*length(X)})
taxa.filter <- taxa.raw[,filter.index1]
taxa_name_list <- colnames(taxa.filter)
zibr_tip_glom_fiber_grp23 <- select(tip_glom_counts_spread_fiber_grp23, c(Immune_group:Timepoint_value)) %>%
  bind_cols(., taxa.filter) %>% 
  na.omit() 
zibr_tip_glom_fiber_grp23 <- zibr_tip_glom_fiber_grp23 %>% 
  mutate(Group_value=ifelse(Immune_group=="group1",0,1))
zibr_tip_glom_fiber_grp23$Timepoint_value %<>% as.integer
zibr_pvalue_df_fiber_grp23 <- zibr_run_function(filter(zibr_tip_glom_fiber_grp23,Timepoint_value!=1),taxa_name_list)
sigg_zibr_fiber_grp23 <- filter(zibr_pvalue_df_fiber_grp23,
         Group_joint_pvalue_adj<=0.05|
         Group_log_pvalue_adj<=0.05|
         Group_beta_pvalue_adj<=0.05) %>% 
  filter(Base_joint_pvalue_adj>=0.05 &
         Base_log_pvalue_adj>=0.05 &
         Base_beta_pvalue_adj>=0.05) #none sig
...
sigg_zibr_tip_glom_fiber_grp13 <- zibr_tip_glom_fiber_grp13 %>% 
  ...
  mutate(bin_present = ifelse(ASV_rel_ab>0,1,0)) %>% 
  right_join(rep_ASV_labels,.)
sigg_zibr_tip_glom_fiber_grp23 <- zibr_tip_glom_fiber_grp23 %>% 
  gather(key=rep_ASV_label,value=ASV_rel_ab,-c(Immune_group:Timepoint_value)) %>% 
  filter(rep_ASV_label %in% sigg_zibr_fiber_grp23$taxa_name) %>% 
  mutate(bin_present = ifelse(ASV_rel_ab>0,1,0)) %>% 
  right_join(rep_ASV_labels,.)
...
ggplot(filter(sigg_zibr_tip_glom_fiber_grp12,Timepoint_value!=1), aes(x=Timepoint_value, y=ASV_rel_ab, fill=Immune_group)) +
  geom_boxplot() + 
...
#ggsave(paste(save_path_figures,"fiber_inflamm_groups_genus_abundance_grp12.pdf"),width = 3.5,height=5,useDingbats=FALSE)

#fiber group 1 vs 3
sigg_zibr_tip_glom_fiber_grp13$Timepoint_value %<>% as.factor()
ggplot(filter(sigg_zibr_tip_glom_fiber_grp13,Timepoint_value!=1), aes(x=Timepoint_value, y=ASV_rel_ab, fill=Immune_group)) +
  geom_boxplot() + 
  ...
  labs(x="Timepoint (months)",y="Relative Abundance")+
  facet_wrap(~rep_ASV_label,scales="free")
#ggsave(paste(save_path_figures,"fiber_inflamm_groups_genus_abundance_grp13.pdf"),width = 7,height=5,useDingbats=FALSE)

#plot presence absence (log regression)
ggplot(filter(sigg_zibr_tip_glom_fiber_bins_grp12,Timepoint_value!=1), aes(x=Timepoint_value, y=percent_present, fill=Immune_group)) +
...
ggplot(filter(sigg_zibr_tip_glom_fiber_bins_grp13,Timepoint_value!=1), aes(x=Timepoint_value, y=percent_present, fill=Immune_group)) +
  geom_bar(position="dodge",stat = "identity") +
  scale_fill_manual(breaks=c("group1","group3"),values=c("#d6604d","#4393c3"))+
  theme_classic()+
  facet_wrap(~rep_ASV_label)+
...
```
Extra plotting for clarity
```{r}
#plot relative abundance (beta regression)
#fiber group 1 vs 2
# sigg_zibr_tip_glom_fiber_grp12_quant <- sigg_zibr_tip_glom_fiber_grp12 %>% 
#   group_by(rep_ASV_label,Timepoint_value) %>% 
#   dplyr::summarize(upper_lim=quantile(ASV_rel_ab,na.rm=T)[4] + 1.5*IQR(ASV_rel_ab,na.rm=T), # x*IQR is variable depending on each plot
#                    lower_lim=quantile(ASV_rel_ab,na.rm=T)[2] - 1*IQR(ASV_rel_ab,na.rm=T))
# sigg_zibr_tip_glom_fiber_grp12$Timepoint_value %<>% as.factor()
# ggplot(filter(sigg_zibr_tip_glom_fiber_grp12,Timepoint_value!=1), aes(x=Timepoint_value, y=ASV_rel_ab, fill=Immune_group)) +
...
#   labs(x="Timepoint (months)",y="Relative Abundance")+
#   facet_wrap(~rep_ASV_label,scales="free")
# ggsave(paste(save_path_figures,"fiber_inflamm_groups_genus_abundance_grp12.pdf"),width = 7,height=5,useDingbats=FALSE)

#fiber group 1 vs 3
sigg_zibr_tip_glom_fiber_grp13 <- sigg_zibr_tip_glom_fiber_grp13 %>% 
  mutate(ASV_rel_ab_percen=ASV_rel_ab*100)
sigg_zibr_tip_glom_fiber_grp13_quant <- sigg_zibr_tip_glom_fiber_grp13 %>% 
  group_by(rep_ASV_label,Timepoint_value) %>% 
  dplyr::summarize(upper_lim=quantile(ASV_rel_ab_percen,na.rm=T)[4] + 4*IQR(ASV_rel_ab_percen,na.rm=T), # x*IQR is variable depending on each plot
                   lower_lim=quantile(ASV_rel_ab_percen,na.rm=T)[2] - 1*IQR(ASV_rel_ab_percen,na.rm=T))
sigg_zibr_tip_glom_fiber_grp13$Timepoint_value %<>% as.factor()
ggplot(filter(sigg_zibr_tip_glom_fiber_grp13), aes(x=Timepoint_value, y=ASV_rel_ab_percen, fill=Immune_group)) +
  geom_boxplot(outlier.shape = NA) + 
  scale_fill_manual(breaks=c("group1","group3"),values=c("#d6604d","#4393c3"))+
  theme_classic()+
  scale_x_discrete(breaks=c(1,2,3,4,5,6,7,8,9),labels=c("-2","0","2", "4","6", "8","10","12","14"),name="Timepoint (weeks)") + 
  ...
  facet_wrap(~rep_ASV_label,scales="free")+
  coord_cartesian(ylim=c(0, max(sigg_zibr_tip_glom_fiber_grp13_quant$upper_lim)))
ggsave(paste(save_path_figures,"fiber_inflamm_groups_genus_abundance_grp13.pdf"),width = 3.5,height=5,useDingbats=FALSE)

#fiber group 2 vs 3
sigg_zibr_tip_glom_fiber_grp23 <- sigg_zibr_tip_glom_fiber_grp23 %>% 
  mutate(ASV_rel_ab_percen=ASV_rel_ab*100)
sigg_zibr_tip_glom_fiber_grp23_quant <- sigg_zibr_tip_glom_fiber_grp23 %>% 
  group_by(rep_ASV_label,Timepoint_value) %>% 
  dplyr::summarize(upper_lim=quantile(ASV_rel_ab_percen,na.rm=T)[4] + .4*IQR(ASV_rel_ab_percen,na.rm=T), # x*IQR is variable depending on each plot
                   lower_lim=quantile(ASV_rel_ab_percen,na.rm=T)[2] - 1*IQR(ASV_rel_ab_percen,na.rm=T))
sigg_zibr_tip_glom_fiber_grp23$Timepoint_value %<>% as.factor()
ggplot(filter(sigg_zibr_tip_glom_fiber_grp23), aes(x=Timepoint_value, y=ASV_rel_ab_percen, fill=Immune_group)) +
  geom_boxplot(outlier.shape = NA) + 
  scale_fill_manual(breaks=c("group2","group3"),values=c("#fdae61","#4393c3"))+
  theme_classic()+
  scale_x_discrete(breaks=c(1,2,3,4,5,6,7,8,9),labels=c("-2","0","2", "4","6", "8","10","12","14"),name="Timepoint (weeks)") + 
  labs(x="Time (months)",y="Relative Abundance (%)")+
  facet_wrap(~rep_ASV_label,scales="free")+
  coord_cartesian(ylim=c(0, max(sigg_zibr_tip_glom_fiber_grp23_quant$upper_lim)))
ggsave(paste(save_path_figures,"fiber_inflamm_groups_genus_abundance_grp23.pdf"),width =3.5,height=5,useDingbats=FALSE)

#plot presence absence (log regression)
sigg_zibr_tip_glom_fiber_bins_grp12 <- sigg_zibr_tip_glom_fiber_bins_grp12 %>% 
  mutate(percent_present_percen=percent_present*100)
sigg_zibr_tip_glom_fiber_bins_grp12$Timepoint_value %<>% as.factor
ggplot(filter(sigg_zibr_tip_glom_fiber_bins_grp12,rep_ASV_label!="ASV_487"), aes(x=Timepoint_value, y=percent_present_percen, fill=Immune_group)) +
  geom_bar(position="dodge",stat = "identity") +
  scale_fill_manual(breaks=c("group1","group2"),values=c("#d6604d","#fdae61"))+
  theme_classic()+
  facet_wrap(~rep_ASV_label)+
  scale_x_discrete(breaks=c(1,2,3,4,5,6,7,8,9),labels=c("-2","0","2", "4","6", "8","10","12","14"),name="Time (weeks)")+
  ylab("% Present in Participants")
ggsave(paste(save_path_figures,"fiber_inflamm_groups_genus_PA_grps12.pdf"),width = 7,height=5,useDingbats=FALSE)
```
...
SCFA baseline pair-wise comparisons 
```{r}
scfa_list <- unique(scfa_data_fiber$Analyte) 

scfa_data_fiber_spread <- scfa_data_fiber %>% 
  spread(key=Analyte,value=Result)

scfa_data_fiber_baseline <- scfa_data_fiber_spread %>% 
  filter(Timepoint==2)
scfa_fiber_baseline_grp1 <- scfa_data_fiber_baseline %>% 
  filter(Immune_group=="group1")
scfa_fiber_baseline_grp2 <- scfa_data_fiber_baseline %>% 
  filter(Immune_group=="group2")
scfa_fiber_baseline_grp3 <- scfa_data_fiber_baseline %>% 
  filter(Immune_group=="group3")

pvalue_list<- c()
for(i in scfa_list){
  pvalue=t.test(scfa_fiber_baseline_grp1[,i],
                scfa_fiber_baseline_grp2[,i],
                paired=FALSE)$p.value
  pvalue_list <- c(pvalue_list,pvalue)
}
scfa_baseline_grp12 <- data.frame(Analyte=scfa_list,
                                     pvalue=pvalue_list)

pvalue_list<- c()
for(i in scfa_list){
  pvalue=t.test(scfa_fiber_baseline_grp1[,i], 
                scfa_fiber_baseline_grp3[,i],
                paired=FALSE)$p.value
  pvalue_list <- c(pvalue_list,pvalue)
}
scfa_baseline_grp13 <- data.frame(Analyte=scfa_list,
                                     pvalue=pvalue_list)
```