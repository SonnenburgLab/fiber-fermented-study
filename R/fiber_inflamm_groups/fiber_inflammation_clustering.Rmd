...
# as of new clustering, there are 5 ppts in group 1, though one is missing its time 6 so only 4
set.seed(20)
...
siggenes_df <- summary(siggenes_output,0.207752)
...
GRP1_feature_counts <- siggenes_df@mat.sig %>%
  filter(q.value <= 0.1) %>%
  mutate(Analyte = rownames(siggenes_df@mat.sig)[c(1:dim(.)[1])]) %>%
  select(Analyte,d.value) 
...
set.seed(20)
pt_count <- length(unique(cytof_sig_fiber_GRP2_1_6$Participant))
siggenes_cl <- c((-1:-pt_count), 1:pt_count)
siggenes_data <- t(cytof_sig_fiber_GRP2_1_6[-c(1:9)])
siggenes_output <- siggenes::sam(data=siggenes_data,cl=siggenes_cl)
summary(siggenes_output)
delta_siggenes <- findDelta(siggenes_output, fdr = 0.05)
siggenes_df <- summary(siggenes_output,1.330219)
siggenes_mat.sig_fiber_cytof_sig_GRP2 <- siggenes_df@mat.sig %>%
  filter(q.value <= 0.1) %>%
  mutate(Analyte = rownames(siggenes_df@mat.sig)[c(1:dim(.)[1])]) %>%
  select(Analyte,q.value)
GRP2_feature_counts <- siggenes_df@mat.sig %>%
  filter(q.value <= 0.1) %>%
  mutate(Analyte = rownames(siggenes_df@mat.sig)[c(1:dim(.)[1])]) %>%
  select(Analyte,d.value) 
...
set.seed(20)
pt_count <- length(unique(cytof_sig_fiber_GRP3_1_6$Participant))
...
siggenes_mat.sig_fiber_cytof_sig_GRP3 <- siggenes_df@mat.sig %>%
  ...
  mutate(Analyte = rownames(siggenes_df@mat.sig)[c(1:dim(.)[1])]) %>%
  select(Analyte,q.value)
GRP3_feature_counts <- siggenes_df@mat.sig %>%
  filter(q.value <= 0.1) %>%
  mutate(Analyte = rownames(siggenes_df@mat.sig)[c(1:dim(.)[1])]) %>%
  select(Analyte,d.value) 
...
cytof_summary <- data.frame(Group = c("group1","group1", "group2","group2", "group3", "group3"), 
           Sig_count = c(sum(GRP1_feature_counts$d.value > 0), -sum(GRP1_feature_counts$d.value < 0),
                                      sum(GRP2_feature_counts$d.value > 0), -sum(GRP2_feature_counts$d.value < 0),
                                      sum(GRP3_feature_counts$d.value > 0), -sum(GRP3_feature_counts$d.value < 0))) %>%
      mutate(Sign = sign(Sig_count))
cytof_summary$Group <- factor(cytof_summary$Group, levels = rev(levels(cytof_summary$Group)))
             
ggplot(cytof_summary, aes(x = Group, y = Sig_count)) + 
  geom_bar(aes(fill = factor(Sign)), stat = "identity", position = "dodge") + theme_bw() + coord_flip()
ggsave(paste(save_path_figures,"fiber_inflamm_groups_cytof_sigg_counts.pdf"),width = 6,height=3,useDingbats=FALSE) 

# counts up and down (not sig and sig): 
grp1_directions <- cytof_sig_fiber_GRP1_1_6 %>% gather(., key = "Feature", value = "value", B_cells_IkB:Classical_monocytes_pZap70) %>%
  group_by(Feature, Timepoint) %>% summarise(mean_val = mean(value)) %>%
  spread(key = Timepoint, value = mean_val) %>%
  mutate(direction = sign(`6` - `1`))

grp2_directions <- cytof_sig_fiber_GRP2_1_6 %>% gather(., key = "Feature", value = "value", B_cells_IkB:Classical_monocytes_pZap70) %>%
  group_by(Feature, Timepoint) %>% summarise(mean_val = mean(value)) %>%
  spread(key = Timepoint, value = mean_val) %>%
  mutate(direction = sign(`6` - `1`))

grp3_directions <- cytof_sig_fiber_GRP3_1_6 %>% gather(., key = "Feature", value = "value", B_cells_IkB:Classical_monocytes_pZap70) %>%
  group_by(Feature, Timepoint) %>% summarise(mean_val = mean(value)) %>%
  spread(key = Timepoint, value = mean_val) %>%
  mutate(direction = sign(`6` - `1`))

cytof_summary_total <- data.frame(Group = c("group1","group1", "group2","group2", "group3", "group3"), 
           Count_total = c(sum(grp1_directions$direction == 1), -sum(grp1_directions$direction == -1),
                                      sum(grp2_directions$direction == 1), -sum(grp2_directions$direction == -1),
                                      sum(grp3_directions$direction == 1), -sum(grp3_directions$direction == -1))) %>%
      mutate(Sign = sign(Count_total))
cytof_summary_total$Group <- factor(cytof_summary$Group, levels = rev(levels(cytof_summary$Group)))

cytof_summary$Sign <- c(1, -1, 1, -1, 1, -1)
combo_counts <- left_join(cytof_summary, cytof_summary_total) %>% mutate(Non_sig = Count_total - Sig_count) %>%
  select(-Count_total) %>% gather(key = "count_type", value = "count", Sig_count, Non_sig)
combo_counts$Group <- factor(combo_counts$Group)

ggplot(combo_counts, aes(x = factor(Sign), y = count)) + 
  geom_bar(aes(fill = factor(count_type)), stat = "identity", position = "stack") + theme_bw() + facet_grid(Group~.) + coord_flip()

ggsave(paste(save_path_figures,"fiber_inflamm_groups_cytof_counts.pdf"),width = 6,height=4,useDingbats=FALSE) 
```
...
fiber_g_intake_inflamm <- fiber_grouping_total_intake %>% 
  right_join(inflammationGroups,.) %>% 
  na.omit()
fiber_g_intake_inflamm$Timepoint <- as.integer(fiber_g_intake_inflamm$Timepoint)
...
fiber_g_intake_GRP1$Timepoint <- as.integer(fiber_g_intake_GRP1$Timepoint)
...
fiber_g_intake_inflamm$Timepoint <- as.factor(fiber_g_intake_inflamm$Timepoint)
...
zibr_tip_glom_fiber_grp12$Timepoint_value <- as.integer(zibr_tip_glom_fiber_grp12$Timepoint_value)
...
zibr_tip_glom_fiber_grp13 <- zibr_tip_glom_fiber_grp13 %>% 
  mutate(Group_value=ifelse(Immune_group=="group1",0,1))
zibr_tip_glom_fiber_grp13$Timepoint_value <- as.integer(zibr_tip_glom_fiber_grp13$Timepoint_value)
...
zibr_tip_glom_fiber_grp23 <- zibr_tip_glom_fiber_grp23 %>% 
  mutate(Group_value=ifelse(Immune_group=="group1",0,1))
zibr_tip_glom_fiber_grp23$Timepoint_value <- as.integer(zibr_tip_glom_fiber_grp23$Timepoint_value)
...
sigg_zibr_tip_glom_fiber_bins_grp13 <- sigg_zibr_tip_glom_fiber_grp13 %>% 
  group_by(Immune_group,Timepoint_value,rep_ASV_label) %>% 
  dplyr::summarize(percent_present=mean(c(bin_present)))
sigg_zibr_tip_glom_fiber_bins_grp23 <- sigg_zibr_tip_glom_fiber_grp23 %>% 
  group_by(Immune_group,Timepoint_value,rep_ASV_label) %>% 
  dplyr::summarize(percent_present=mean(c(bin_present)))

#plot relative abundance (beta regression)
#fiber group 1 vs 2
sigg_zibr_tip_glom_fiber_grp12$Timepoint_value <- as.factor(sigg_zibr_tip_glom_fiber_grp12$Timepoint_value)
...
#fiber group 1 vs 3
sigg_zibr_tip_glom_fiber_grp13$Timepoint_value <- as.factor(sigg_zibr_tip_glom_fiber_grp13$Timepoint_value)
...
#fiber group 2 vs 3
sigg_zibr_tip_glom_fiber_grp23$Timepoint_value <- as.factor(sigg_zibr_tip_glom_fiber_grp23$Timepoint_value)
ggplot(filter(sigg_zibr_tip_glom_fiber_grp23,Timepoint_value!=1), aes(x=Timepoint_value, y=ASV_rel_ab, fill=Immune_group)) +
  geom_boxplot() + 
...
write.csv(sigg_zibr_fiber_grp23,paste(save_path_figures,"sigg_zibr_tip_glom_fiber_inflamm_group_23_table.csv"))

tax_IDs <- bind_rows(sigg_zibr_tip_glom_fiber_grp12 %>% select(rep_ASV_label:Species) %>% unique(.),
          sigg_zibr_tip_glom_fiber_grp13 %>% select(rep_ASV_label:Species) %>% unique(.),
          sigg_zibr_tip_glom_fiber_grp23 %>% select(rep_ASV_label:Species) %>% unique(.))

write.csv(tax_IDs,paste(save_path_figures,"sigg_zibr_ASV_tax_IDs.csv"))
